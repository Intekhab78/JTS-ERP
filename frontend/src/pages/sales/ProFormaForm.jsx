...
const ProFormaForm = () => {
  ...
  const fetchProForma = async () => {
    try {
      ...
      console.error('Failed to fetch Pro-Forma invoice', error);
  ...
  const handlePercentageChange = (e) => {
    ...
    if (val === '') {
      setFormData({ ...formData, paymentRequestPercentage: '', paymentRequestAmount: '' });
      return;
    }
    ...
    const amount = gt > 0 ? Number((gt * (percentage / 100)).toFixed(2)) : 0;
    
    // Convert back to string to drop leading zeros for UI cleanly if they type something weird
    setFormData({ ...formData, paymentRequestPercentage: percentage.toString(), paymentRequestAmount: amount });
  };

  const handleAmountChange = (e) => {
    const val = e.target.value;
    if (val === '') {
      setFormData({ ...formData, paymentRequestAmount: '', paymentRequestPercentage: '' });
      return;
    }
    let amount = Number(val);
    const gt = calculateGrandTotal();
    if (amount < 0) amount = 0;
    if (gt > 0 && amount > gt) amount = gt;
    const percentage = gt > 0 ? Number(((amount / gt) * 100).toFixed(2)) : 100;
    setFormData({ ...formData, paymentRequestAmount: amount.toString(), paymentRequestPercentage: percentage });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(error.response?.data?.message || 'Failed to save Pro-Forma Invoice');
    } finally {
      setLoading(false);
    }
  };
  ...
  const isReadOnly = (isEditing && formData.status && formData.status !== 'DRAFT') || 
                     (isEditing && !hasPermission('EDIT_PROFORMA')) || 
                     (!isEditing && !hasPermission('CREATE_PROFORMA'));

  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/sales/proforma')} className="p-2 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
            <ArrowLeft className="h-5 w-5 text-slate-600" />
          </button>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">
              {isEditing ? `Pro-Forma Invoice ${formData.pfNumber || ''}` : 'New Pro-Forma Invoice'}
            </h1>
        ...
        <div className="flex gap-3">
          {isEditing && hasPermission('PRINT_PROFORMA') && (
            <Button type="button" variant="outline" onClick={() => navigate(`/sales/proforma/print/${id}`)} leftIcon={<Printer className="w-4 h-4" />}>
              Print
            </Button>
          )}
          {!isReadOnly && (
            <>
              <Button type="button" variant="outline" onClick={() => navigate('/sales/proforma')} leftIcon={<X className="w-4 h-4" />}>
                Cancel
              </Button>
            ...
            <div className="space-y-4">
              ...
              {formData.sourceType === 'QUOTE' && (
                <div className="mt-6 pt-6 border-t-2 border-slate-200">
                  <h4 className="text-md font-bold text-slate-800 mb-4">Billing Request (Partial/Progressive)</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1">Request Percentage (%)</label>
                      <input
                        type="number"
                        min="0"
                        max="100"
                        step="0.01"
                        value={formData.paymentRequestPercentage !== undefined ? formData.paymentRequestPercentage : ''}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1">Request Amount ({currencySymbol})</label>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                        value={formData.paymentRequestAmount !== undefined ? formData.paymentRequestAmount : ''}
                  ...
                  <div className="flex justify-between items-center p-4 bg-blue-50 rounded-xl border border-blue-100">
                    <span className="text-lg font-bold text-blue-900">Total Requested</span>
                    <span className="text-3xl font-black text-blue-600">
                      {currencySymbol}{(Number(formData.paymentRequestAmount) || 0).toFixed(2)}
                    </span>
                  </div>
                </div>
              )}
